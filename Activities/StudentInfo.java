...
public class StudentInfo {
    ...
    public void promptName() {
        ...
        while (true) {
            ...
            if (userInput.isEmpty()) errorMessage("Invalid input, name must not be blank");
            else break;
        }
    ...
    public void promptGrade() {
        ...
        while (true) {
            try {
                System.out.println("Enter GWA (1.00, 1.25, ..., 3.00, 5.00): ");
                ...
                gwa == 3.00f || gwa == 5.00f) { break; }
                else {
                    errorMessage("Invalid input, please enter a valid GWA (1.00-3.00 or 5.00)");
                }
            } catch (InputMismatchException e) {
                    errorMessage("Invalid input, pleae enter a decimal number (e.g. 1.25)");
                    scanner.nextLine();
            }
        }
        ...
        if (gwa == 1.25f) convertedPercentage = 96;
        if (gwa == 1.50f) convertedPercentage = 93;
        if (gwa == 1.75f) convertedPercentage = 90;
        if (gwa == 2.00f) convertedPercentage = 87;
        if (gwa == 2.25f) convertedPercentage = 84;
        if (gwa == 2.50f) convertedPercentage = 81;
        if (gwa == 2.75f) convertedPercentage = 78;
        if (gwa == 3.00f) convertedPercentage = 75;
        if (gwa == 5.00f) convertedPercentage = 65;
    ...
    public void promptIsEnrolled() {
        ...
        while (true) {
            System.out.println("Are you enrolled? [y/n]: ");
            input = scanner.nextLine();

            if (input.isEmpty()) errorMessage("Invalid input, enrollment status cannot be empty");
